                ...
                <div class="card-body">
                    <form action="@if (Route::currentRouteName() == 'series.edit'){{ route('series.update', ['series' => $room->id]) }}@else{{ route('series.store') }}@endif" method="post">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control @error('name') is-invalid @enderror" name="name" value="@if (Route::currentRouteName() == 'series.edit'){{ $room->name }}@else{{ old('name') }}@endif" placeholder="Digite a Série">
                            @error('name')
